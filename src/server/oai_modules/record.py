...
class RecordManager(object):
    ...
    def get_records(self, metadata_format, only_headers, resumption_token, range_from, range_until, set_type, set_id, limit=100):
        ...
        scroll_info = ScrollInfo.parse(resumption_token, range_from, range_until, set_type,
                                       set_id, metadata_format.prefix if metadata_format is not None else None)
        ...
        if scroll_info.set_type is not None:
            if scroll_info.set_type == 'pool':
                search_elements.append(
                    {'type': 'in', 'objecttype': '_pool', 'in': [scroll_info.set_id]})
            elif scroll_info.set_type == 'objecttype_pool':
                try:
                    search_elements.append(
                        {'type': 'in', 'objecttype': '_pool', 'in': [set_id[0]]})
                ...
                search_elements.append(
                    {'type': 'in', 'fields': ['_collections._id'], 'in': [scroll_info.set_id]})
    ...
    def _parse_record(self, object_js, metadata_info=None):
        try:
            ...
            record = Record(self.repository, uuid)
            sets = context.get_json_value(
                object_js, '_sets') + self._get_tagfilter_sets_for_object(context.get_json_value(object_js, '_tags'))
            try:
